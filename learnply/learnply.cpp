...
//CUSTOM VARS HERE:
double const TINY_RADIUS = 0.5;
int const CIRCLE_POINTS = 10;//TODO: make bigger
...
void morse_smale();
void update_scalars();
std::vector<Quad*>get_quads_for_point(double x, double y);
/******************************************************************************
Main program.
******************************************************************************/
...
******************************************************************************/
double det_hessian_matrix(Quad* quad)
{		//TODO: Update for boundary cases if time.
 ...
 //Loop through all min vertices
	...
	for (int i = 0; i < mins.size(); i++) {
		...
		for (int j = 0; j < CIRCLE_POINTS; j ++) {	
			...
			points.push_back(icVector3(x, y, fxy));
			...
			LineSegment ls = LineSegment(x, y, fxy, next_x, next_y, f_next_xy);
			PolyLine lin;
			lin.push_back(ls);
			lines.push_back(lin);
			PolyLine pl;
		...
		for (int j = 0; j < saddles.size(); j++) {
				x = x0 + (saddles.at(j).x - x0)*STEP_SIZE;
				y = y0 + (saddles.at(j).y - y0)*STEP_SIZE;
				quad_xy = get_quad_by_position(x, y);
				double fxy = bilinear_interpolate_at_point(quad_xy, x, y);
				icVector2 grad = normalized_grad_at_point(quad_xy, x, y);
		...
		for (int k = 0; k < saddles.size(); k++) {
			...
			bool add = false;
			for (int j = 0; j < CIRCLE_POINTS + saddles.size(); j++) {
...
{
		std::vector<Quad*> quads = get_quads_for_point(x0, y0);
		Vertex* v1 = quads.at(0)->verts[0];
		Vertex* v2 = quads.at(0)->verts[1];
		Vertex* v3 = quads.at(0)->verts[2];
		Vertex* v4 = quads.at(0)->verts[3];
		...
		Vertex* x1y1 = find_with_coords(x1, y1, 4, quads.at(0)->verts);
		Vertex* x2y1 = find_with_coords(x2, y1, 4, quads.at(0)->verts);
		Vertex* x1y2 = find_with_coords(x1, y2, 4, quads.at(0)->verts);
		Vertex* x2y2 = find_with_coords(x2, y2, 4, quads.at(0)->verts);
		...
		if (x1 <= x0 && x2 >= x0) {
			if (y1 <= y0 && y2 >= y0) {
				...
				double biggest = std::max(std::max(fx1y1, fx2y2), std::max(fx1y2, fx2y1));
				...
				icVector2 grad = icVector2(x_dir, y_dir);

				if (quads.size() > 1) {
					v1 = quads.at(1)->verts[0];
					v2 = quads.at(1)->verts[1];
					v3 = quads.at(1)->verts[2];
					v4 = quads.at(1)->verts[3];

					x1 = std::min(std::min(v3->x, v1->x), std::min(v2->x, v4->x));
					x2 = std::max(std::max(v3->x, v1->x), std::max(v2->x, v4->x));
					y1 = std::min(std::min(v3->y, v1->y), std::min(v2->y, v4->y));
					y2 = std::max(std::max(v3->y, v1->y), std::max(v2->y, v4->y));

					x1y1 = find_with_coords(x1, y1, 4, quads.at(1)->verts);
					x2y1 = find_with_coords(x2, y1, 4, quads.at(1)->verts);
					x1y2 = find_with_coords(x1, y2, 4, quads.at(1)->verts);
					x2y2 = find_with_coords(x2, y2, 4, quads.at(1)->verts);
					...
					biggest = std::max(std::max(fx1y1, fx2y2), std::max(fx1y2, fx2y1));
					...
					grad.x = (grad.x + x_dir) / 2;
					grad.y = (grad.y + y_dir) / 2;
				}

				normalize(grad);
				//return average for boundaries.
				return grad;
			}
		}
...
void display_polyhedron(Polyhedron* poly)
{
	...
	case 2:	//morse smale + points + height
	{
		...
		{
			icVector3 point = saddles[k];
			drawDot(point.x, point.y, point.z, 0.03, 0, 0, 1);
		}
		// draw points
		for (int k = 0; k < mins.size(); k++)
		{
			icVector3 point = mins[k];
			drawDot(point.x, point.y, point.z, .03, 1, 0, 0);
		}
		// draw points
		for (int k = 0; k < max.size(); k++)
		{
			icVector3 point = max[k];
			drawDot(point.x, point.y, point.z, 0.03, 0, 1, 0);
		}
		for (int k = 0; k < points.size(); k++) {
			icVector3 pt = points[k];
			drawDot(pt.x, pt.y, pt.z, 0.01, 0, 0, 0);
		}
		// draw lines
		for (int k = 0; k < morse_smale_lines.size(); k++)
		{
			drawPolyLine(morse_smale_lines[k], 1.0, 0, 0.0, 0.0);
		}
		for (int k = 0; k < lines.size(); k++) {
			//drawPolyLine(lines[k], 1, .3, 0, .2);
		}
	}
	break;
	default:
